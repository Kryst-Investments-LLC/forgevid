...
async function handleSearchTemplates(body) {
...
async function handleSearchMedia(body) {
    const filters = body.filters || {};
    try {
...
async function handleGetTemplate(body) {
...
async function handleGetMedia(body) {
    const { assetId } = body;
    if (!assetId) {
...
async function handleGenerateTemplate(body) {
    const { prompt } = body;
    if (!prompt) {
...
async function handlePopularTemplates(body) {
    const { limit = 10 } = body;
    try {
...
async function handleRecentTemplates(body) {
    const { limit = 10 } = body;
    try {
...
async function handleRecommendedTemplates(body) {
    const { limit = 10 } = body;
    try {
