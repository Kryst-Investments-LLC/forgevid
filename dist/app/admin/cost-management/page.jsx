...
import { DollarSign, TrendingUp, TrendingDown, AlertTriangle, CheckCircle, XCircle, Lightbulb, Target, } from "lucide-react";
export default function CostManagementPage() {
    ...
    return (<div className="space-y-6">
      ...
      {/* Budget Alerts */}
      {budgetAlerts.length > 0 && (<div className="space-y-3">
          {budgetAlerts.map((alert) => (<div key={alert.id} className={`p-4 rounded-lg border-l-4 ${alert.severity === "critical" ? "bg-red-50 border-red-500" : "bg-yellow-50 border-yellow-500"}`}>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {costData.trends.map((trend) => (<div key={trend.service} className="flex items-center justify-between">
                      <span className="font-medium">{trend.service}</span>
                      <div className="flex items-center space-x-2">
                        {trend.change > 0 ? (<TrendingUp className="h-4 w-4 text-red-500"/>) : (<TrendingDown className="h-4 w-4 text-green-500"/>)}
        ...
        <TabsContent value="cost-centers" className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            {Object.values(costData.costByCenter).map((center) => (<Card key={center.name}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle>{center.name}</CardTitle>
                    <Badge variant={center.utilization >= 95 ? "destructive" : center.utilization >= 85 ? "secondary" : "default"} className={getUtilizationBgColor(center.utilization)}>
        ...
        <TabsContent value="optimizations" className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {costData.optimizations.map((optimization) => (<div key={optimization.id} className="p-4 border rounded-lg">
                    <div className="flex items-start justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          <Badge variant="outline">{optimization.type.replace("_", " ")}</Badge>
                          <Badge variant={optimization.effort === "low"
                ? "default"
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                {Object.values(costData.costByCenter).map((center) => (<div key={center.name} className="space-y-3">
