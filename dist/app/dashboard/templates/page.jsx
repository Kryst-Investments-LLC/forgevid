    ...
    return (<div className="min-h-[600px]">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-16">
            {templates.map((template) => (<Card key={template.id} className="transition-transform duration-300 hover:scale-105 hover:shadow-2xl bg-white/90 border-0 rounded-2xl overflow-hidden" style={{ boxShadow: "0 2px 16px rgba(79,70,229,0.08)" }}>
                ...
                <CardContent className="pt-2">
                  <img src={template.thumbnail} alt={template.title} width={320} height={160} className="w-full h-40 object-cover rounded-xl border border-gray-200 mb-4 shadow-sm aspect-[2/1]"/>
                  <div className="flex items-center justify-between mt-2">
                    <Button onClick={() => handlePreviewTemplate(template)} className="bg-gradient-to-r from-indigo-500 to-cyan-400 text-white font-semibold px-6 py-2 rounded-lg shadow hover:from-indigo-600 hover:to-cyan-500">
                      Preview
                    </Button>
                    {template.isPro && (<Badge variant="destructive" className="ml-2 bg-gradient-to-r from-pink-500 to-red-400 text-white shadow">Pro</Badge>)}
                  </div>
                </CardContent>
              </Card>))}
        </div>
      ...
      {selectedTemplate && (<VideoPlayerModal video={{
                ...
                size: "",
                format: "MP4"
            }} isOpen={isPreviewOpen} onClose={handleClosePreview}/>)}
    </div>);
}
