...
import { Sparkles, Wand2, Brain, Heart, Zap, Clock, Play, Download, Eye, TrendingUp, Star, X } from "lucide-react";
...
export default function AIFeaturesPage() {
    ...
    return (<div className="max-w-6xl mx-auto">
          ...
          <Tabs defaultValue="create" className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea placeholder="Create a product advertisement for eco-friendly running shoes. Show athletes in nature, emphasize sustainability, upbeat music, modern transitions..." value={prompt} onChange={(e) => setPrompt(e.target.value)} className="min-h-[120px]"/>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <Slider value={videoLength} onValueChange={setVideoLength} min={15} max={300} step={15} className="w-full"/>
        ...
        ].map((style) => (<Button key={style.id} variant="outline" className={`h-auto p-3 flex flex-col items-start transition-all ${selectedStyle === style.id
                ? 'border-cyan-400 bg-cyan-400/10 text-white'
                : 'border-gray-600 bg-gray-800/50 text-gray-300 hover:border-gray-500 hover:bg-gray-700/50'}`} onClick={() => setSelectedStyle(style.id)}>
            ...
            { id: "effects", name: "Smart Effects", desc: "Emotion-aware transitions" },
        ].map((addon) => (<div key={addon.id} className="flex items-center space-x-3">
                            <Checkbox id={addon.id} checked={selectedAddOns.includes(addon.id)} onCheckedChange={(checked) => handleAddOnChange(addon.id, checked)}/>
                    ...
                    {/* Generate Button */}
                    <Button className="w-full" size="lg" onClick={handleGenerate} disabled={!prompt.trim() || isGenerating}>
                      {isGenerating ? (<>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="aspect-video bg-muted rounded-lg flex items-center justify-center mb-4 overflow-hidden relative">
                      {generatedVideo ? (<div className="relative w-full h-full">
                          <video key={generatedVideo} controls autoPlay className="w-full h-full object-cover rounded-lg" poster="https://via.placeholder.com/640x360/1f2937/ffffff?text=AI+Generated+Video" preload="auto" onClick={() => setIsVideoFullscreen(true)}>
                            <source src={generatedVideo} type="video/mp4"/>
                            Your browser does not support the video tag.
                          </video>
                          <Button variant="outline" size="sm" className="absolute top-2 right-2 bg-black/50 hover:bg-black/70 text-white border-white/20" onClick={() => setIsVideoFullscreen(true)}>
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" className="flex-1 bg-transparent" disabled={!generatedVideo} onClick={() => setIsVideoFullscreen(true)}>
                        <Play className="h-4 w-4 mr-2"/>
                        Preview
                      </Button>
                      <Button variant="outline" className="flex-1 bg-transparent" disabled={!generatedVideo} onClick={() => {
            if (generatedVideo) {
                ...
                link.click();
                      ...
                      {generatedScript && (<Button variant="outline" className="flex-1 bg-transparent" onClick={() => {
                console.log('========================================');
                ...
                toast.success('✅ Script logged to console! Press F12 to view your custom script.');
                      ...
                      {generatedVideo && (<Button variant="outline" className="flex-1 bg-cyan-600 hover:bg-cyan-700 text-white" onClick={() => {
        ...
        ].map((item) => (<div key={item.emotion} className="space-y-1">
                            <div className="flex justify-between text-sm">
          ...
          {isVideoFullscreen && generatedVideo && (<div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-8" onClick={() => setIsVideoFullscreen(false)}>
              <Button variant="outline" size="sm" className="absolute top-4 right-4 z-20 bg-black/70 hover:bg-black/90 text-white border-white/30 rounded-full w-10 h-10 p-0" onClick={() => setIsVideoFullscreen(false)}>
                <X className="h-5 w-5"/>
              </Button>
              <video controls autoPlay className="w-full h-auto max-w-6xl rounded-lg shadow-2xl" poster="https://via.placeholder.com/640x360/1f2937/ffffff?text=AI+Generated+Video" onClick={(e) => e.stopPropagation()}>
                <source src={generatedVideo} type="video/mp4"/>
                Your browser does not support the video tag.
              </video>
            </div>)}
