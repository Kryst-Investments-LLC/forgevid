...
class ProductionRedis {
  ...
  async createRoom(roomId: string, metadata: any): Promise<void> {
    const roomData = {
      id: roomId,
      ...metadata,
      createdAt: Date.now(),
  ...
  async saveEdit(roomId: string, edit: any, userId: string): Promise<string> {
  ...
  async getEditHistory(roomId: string, fromVersion: number, toVersion: number): Promise<any[]> {
    const edits = await this.redis.lrange(`room:${roomId}:edits`, 0, -1);
    const parsedEdits = edits.map(edit => JSON.parse(edit));
    return parsedEdits.filter(edit => 
      edit.version >= fromVersion && edit.version <= toVersion
    );
  }

  // Message management with threading
  async saveMessage(roomId: string, message: any, userId: string, threadId?: string): Promise<string> {
  ...
  async getRecentMessages(roomId: string, limit: number = 10, threadId?: string): Promise<any[]> {
    const key = threadId ? `room:${roomId}:thread:${threadId}:messages` : `room:${roomId}:messages`;
  ...
  async saveVideoState(roomId: string, state: any): Promise<void> {
    const stateData = {
      ...state,
      timestamp: Date.now(),
      roomId
  ...
  async getVideoState(roomId: string): Promise<any> {
