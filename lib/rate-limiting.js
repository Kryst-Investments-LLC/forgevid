...
class RateLimiter {
    ...
    async checkLimit(req, configName = 'api', customKey) {
    ...
    async getStatus(key, configName = 'api') {
        const config = this.configs.get(configName);
        if (!config) {
            throw new Error(`Rate limit configuration '${configName}' not found`);
        }
    ...
    async resetLimit(key, configName = 'api') {
        const config = this.configs.get(configName);
        if (!config) {
            throw new Error(`Rate limit configuration '${configName}' not found`);
        }
...
export class SecurityMiddleware {
    ...
    static validateFileUpload(file, maxSize, allowedTypes) {
