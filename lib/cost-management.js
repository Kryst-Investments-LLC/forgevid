export class CostManager {
    ...
    static recordCost(service, cost, usage, unit, metadata = {}) {
        const metric = {
            timestamp: Date.now(),
            service,
            cost,
    ...
    static updateCostCenterSpending(service, cost) {
        ...
        for (const [id, costCenter] of this.costCenters.entries()) {
            ...
            for (const alert of costCenter.alerts) {
                if (utilizationPercentage >= alert.threshold) {
                    ...
                    const recentAlert = this.budgetAlerts.find((a) => a.costCenterId === id &&
    ...
    static async triggerBudgetAlert(alertId, costCenter, threshold, current, recipients) {
        const alert = {
            id: alertId,
    ...
    static async sendBudgetAlert(alertData) {
        try {
    ...
    static async sendCostMetric(metric) {
        try {
    ...
    static analyzeCostTrends() {
        ...
        // Group by service
        const serviceSpending = recentMetrics.reduce((acc, metric) => {
    ...
    static detectAnomalousSpending(currentSpending) {
    ...
    static async createCostAlert(service, current, expected) {
        const alert = {
    ...
    static generateOptimizationRecommendations() {
        ...
        if (computeMetrics.length > 0) {
            const avgUtilization = computeMetrics.reduce((sum, m) => sum + (m.metadata.utilization || 0), 0) / computeMetrics.length;
    ...
    static getCostReport(period = "month") {
        const now = Date.now();
        const periodMs = {
            day: 24 * 60 * 60 * 1000,
        ...
        const costByService = periodMetrics.reduce((acc, m) => {
    ...
    static getCostCenterSpending(costCenterId) {
        const costCenter = this.costCenters.get(costCenterId);
        return costCenter ? costCenter.spent : 0;
    }
    static updateBudget(costCenterId, newBudget) {
        const costCenter = this.costCenters.get(costCenterId);
        if (costCenter) {
    ...
    static implementOptimization(optimizationId) {
    ...
    static dismissOptimization(optimizationId) {
        const optimization = this.optimizations.find((opt) => opt.id === optimizationId);
        if (optimization) {
...
export class UsageBasedBilling {
    ...
    static calculateUsageCost(service, usage) {
    ...
    static generateInvoice(userId, period) {
        ...
        const lineItems = Object.entries(usageData).map(([service, usage]) => {
            const cost = this.calculateUsageCost(service, usage);
            const pricing = this.pricingTiers[service];
            return {
                service,
                usage,
