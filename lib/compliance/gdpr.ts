...
export class GDPRCompliance {
  ...
  static async handleDataAccessRequest(userId: string): Promise<any> {
    try {
      ...
      Logger.error('Failed to process data access request', error as Error, { userId });
      throw error;
    }
  }
   ...
   */
  static async handleDataRestrictionRequest(
    userId: string, 
    ...
    try {
      ...
      Logger.error('Failed to process data restriction request', error as Error, { userId });
      throw error;
    }
  }
   ...
   */
  private static async logGDPRRequest(request: GDPRRequest): Promise<void> {
   ...
   */
  static async getComplianceStatus(userId: string): Promise<any> {
   ...
   */
  static async recordConsent(userId: string, consentData: any): Promise<void> {
    await prisma.user.update({
      where: { id: userId },
      data: {
        metadata: JSON.stringify({
