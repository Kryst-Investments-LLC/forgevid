...
export async function POST(req: NextRequest) {
  try {
    ...
    const thumbnailUrl = (uploadResult as any).secure_url.replace('.mp4', '.jpg');
...
export async function PUT(req: NextRequest) {
  try {
    ...
    await prisma.video.update({
      where: { id: videoId },
      data: {
        transcript: action === 'transcribe' ? (result as any).transcript : video.transcript,
        summary: action === 'summarize' ? (result as any).summary : video.summary,
        metadata: JSON.stringify({
