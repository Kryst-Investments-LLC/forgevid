...
export async function GET(request: NextRequest) {
  try {
    ...
    Logger.error('Enterprise features API error', error as Error);
...
// GDPR handlers
async function handleGDPRRequest(request: NextRequest, userId: string) {
  const { searchParams } = new URL(request.url);
...
async function handleGDPRAction(request: NextRequest, userId: string, body: any) {
  const { searchParams } = new URL(request.url);
  const action = searchParams.get('action');
...
// SOC2 handlers
async function handleSOC2Request(request: NextRequest, userId: string) {
  const { searchParams } = new URL(request.url);
  const action = searchParams.get('action');
...
async function handleSOC2Action(request: NextRequest, userId: string, body: any) {
...
async function handlePerformanceRequest(request: NextRequest, userId: string) {
  const { searchParams } = new URL(request.url);
  const action = searchParams.get('action');
...
async function handlePerformanceAction(request: NextRequest, userId: string, body: any) {
  const { searchParams } = new URL(request.url);
  const action = searchParams.get('action');
...
// Health handlers
async function handleHealthRequest(request: NextRequest, userId: string) {
  const { searchParams } = new URL(request.url);
  const action = searchParams.get('action');
