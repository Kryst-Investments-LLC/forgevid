...
describe('/api/health', () => {
  ...
  describe('GET /api/health', () => {
    it('should return healthy status when all services are working', async () => {
      ...
      expect(data.health.checks.every((check: any) => check.status === 'healthy')).toBe(true);
    ...
    it('should return unhealthy status when database is down', async () => {
      ...
      expect(data.health.checks.find((check: any) => check.service === 'database').status).toBe('unhealthy');
    });
    ...
    it('should return unhealthy status when Redis is down', async () => {
      ...
      expect(response.status).toBe(503);
      expect(data.status).toBe('unhealthy');
      expect(data.health.checks.find((check: any) => check.service === 'redis').status).toBe('unhealthy');
    });
    ...
    it('should return degraded status when external APIs are down', async () => {
      ...
      expect(response.status).toBe(503);
      expect(data.status).toBe('unhealthy');
      expect(data.health.checks.find((check: any) => check.service === 'external-apis').status).toBe('unhealthy');
    });
