      ...
      <div className="flex-1 flex">
        ...
        <div className="flex-1 relative overflow-x-auto">
          ...
          <div className="h-8 border-b bg-muted/30 relative">
            {Array.from({ length: 18 }, (_, i) => (<div key={i} className="absolute top-0 h-full border-l border-border/50" style={{ left: `${i * 60}px` }}>
          ...
          {/* Playhead */}
          <div className="absolute top-0 bottom-0 w-0.5 bg-primary z-10 pointer-events-none" style={{ left: `${currentTime * 6}px` }}/>

          {/* Video Track */}
          <div className="h-12 border-b relative bg-background/50">
            {timelineData.video.map((clip) => (<div key={clip.id} className={`absolute top-1 bottom-1 ${clip.color} rounded cursor-pointer hover:opacity-80 flex items-center px-2`} style={{
          ...
          {/* Audio Track */}
          <div className="h-12 border-b relative bg-background/50">
            {timelineData.audio.map((clip) => (<div key={clip.id} className={`absolute top-1 bottom-1 ${clip.color} rounded cursor-pointer hover:opacity-80 flex items-center px-2`} style={{
                left: `${clip.start * 6}px`,
                width: `${clip.duration * 6}px`,
            }}>
                <span className="text-xs text-white font-medium truncate">{clip.name}</span>
          ...
          {/* Text Track */}
          <div className="h-12 border-b relative bg-background/50">
            {timelineData.text.map((clip) => (<div key={clip.id} className={`absolute top-1 bottom-1 ${clip.color} rounded cursor-pointer hover:opacity-80 flex items-center px-2`} style={{
                left: `${clip.start * 6}px`,
                width: `${clip.duration * 6}px`,
            }}>
                <span className="text-xs text-white font-medium truncate">{clip.name}</span>
