...
export function ExportPanel() {
    ...
    return (<UsageLimit feature="exports" currentUsage={currentExports}>
      <Card>
        <CardHeader>
        ...
        ].map((option) => (<div key={option.name}>
                  {option.requiresPro ? (<FeatureGate feature="aiFeatures" plan="pro">
                      <Button variant="outline" className="w-full justify-between h-auto p-3 bg-transparent" onClick={() => handleExport(option.name)} disabled={isExporting}>
                    ...
                    </FeatureGate>) : (<Button variant="outline" className="w-full justify-between h-auto p-3 bg-transparent" onClick={() => handleExport(option.name)} disabled={isExporting}>
                      <div className="text-left">
          ...
          <div>
            ...
            <div className="space-y-2">
              <label className="flex items-center space-x-2 text-sm">
                <input type="checkbox" className="rounded" defaultChecked={!hasFeature("watermark")} disabled={!hasFeature("watermark")}/>
                <span>Include watermark</span>
                {!hasFeature("watermark") && (<Badge variant="outline" className="text-xs">
                    Free
                  </Badge>)}
              </label>
              <label className="flex items-center space-x-2 text-sm">
