...
export function CollaborationRoom({ roomId, user, onError }: CollaborationRoomProps) {
  ...
  useEffect(() => {
    ...
    const connectToCollaboration = async () => {
      try {
        const tokenResponse = await fetch('/api/auth/collaboration-token', {
          method: 'GET',
          cache: 'no-store',
          credentials: 'include',
        });
        ...
        newSocket.on('error', (socketError) => {
          ...
          onError?.(socketError.message || 'Connection error');
        });
        ...
        console.error('Collaboration connection failed:', connectError);
        onError?.('Unable to connect to collaboration server');
      }
    };
