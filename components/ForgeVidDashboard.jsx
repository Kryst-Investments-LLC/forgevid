    ...
    useEffect(() => {
        async function fetchHyperMind() {
            ...
            try {
                ...
                if (!res.ok)
                    throw new Error("Failed to fetch HyperMind data");
        ...
        <main>
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <input type="text" placeholder="Search videos, templates..." className="pl-10 pr-4 py-2 bg-gray-800/50 border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent"/>
              </div>
          ...
          <div className="mb-16 h-[900px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 h-[800px] min-w-full">
              {templatesLoading ? (Array.from({ length: 3 }).map((_, i) => (<div key={i} className="glass-card rounded-2xl overflow-hidden border border-white/6 min-h-[260px] animate-pulse bg-gray-800/40">
                  ...
                  </div>))) : templatesError ? (<div className="col-span-3 text-center text-red-400 py-8">{templatesError}</div>) : templates.length > 0 ? (templates.map((t) => (<div key={t.id} className="glass-card rounded-2xl overflow-hidden border border-white/6 min-h-[260px]">
                  ...
                  </div>))) : (Array.from({ length: 3 }).map((_, i) => (<div key={i} className="glass-card rounded-2xl overflow-hidden border border-white/6 min-h-[260px] bg-gray-800/40">
                    <div className="w-full h-40 bg-gray-700"/>
                    <div className="p-4">
                      <div className="h-5 w-2/3 bg-gray-700 rounded mb-2"/>
                      <div className="h-4 w-1/2 bg-gray-700 rounded mb-2"/>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" style={{ minHeight: '700px' }}>
            {videosLoading ? (Array.from({ length: 3 }).map((_, i) => (<div key={i} className="glass-card rounded-2xl overflow-hidden border border-white/6 min-h-[220px] animate-pulse bg-gray-800/40">
                  <div className="w-full h-40 bg-gray-700"/>
                  <div className="p-4">
                    <div className="h-5 w-2/3 bg-gray-700 rounded mb-2"/>
                    <div className="h-4 w-1/2 bg-gray-700 rounded mb-2"/>
                  </div>
                </div>))) : videosError ? (<div className="col-span-3 text-center text-red-400 py-8">{videosError}</div>) : videos.length > 0 ? (videos.map((v) => (<VideoCard key={v.id} title={v.title} subtitle={v.description || v.summary || ''} thumbnail={v.thumbnail || v.fileUrl || '/placeholder.svg'} status={v.status} views={v.analytics?.[0]?.views || 0} duration={formatDuration(v.duration)} width={320} height={180} aspectRatio="16/9"/>))) : (Array.from({ length: 3 }).map((_, i) => (<div key={i} className="glass-card rounded-2xl overflow-hidden border border-white/6 min-h-[220px] bg-gray-800/40">
                  <div className="w-full h-40 bg-gray-700"/>
                  <div className="p-4">
                    <div className="h-5 w-2/3 bg-gray-700 rounded mb-2"/>
                    <div className="h-4 w-1/2 bg-gray-700 rounded mb-2"/>
          ...
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12" style={{ minHeight: '130px', contain: 'layout' }}>
              {loading ? (Array.from({ length: 4 }).map((_, i) => (<div key={i} className="glass-card p-5 rounded-2xl shadow-lg border border-white/6 flex flex-col h-[110px] min-w-[200px] animate-pulse bg-gray-800/40">
                    <div className="h-4 w-1/3 bg-gray-700 rounded mb-2"/>
                    <div className="h-8 w-1/2 bg-gray-700 rounded"/>
                  </div>))) : error ? (<div className="col-span-4 text-center text-red-400 py-8">{error}</div>) : collaborationMetrics.length > 0 ? (collaborationMetrics.map((m, i) => (<div key={i} className="h-[110px] min-w-[200px]" style={{ contain: 'strict', contentVisibility: 'auto' }}>
                    <MetricCard title={m.label} value={m.value} delta={m.trend}/>
                  </div>))) : (Array.from({ length: 4 }).map((_, i) => (<div key={i} className="glass-card p-5 rounded-2xl shadow-lg border border-white/6 flex flex-col h-[110px] min-w-[200px] bg-gray-800/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12" style={{ minHeight: '130px', contain: 'layout' }}>
              {loading ? (Array.from({ length: 3 }).map((_, i) => (<div key={i} className="glass-card p-5 rounded-2xl shadow-lg border border-white/6 flex flex-col h-[110px] min-w-[200px] animate-pulse bg-gray-800/40">
                    <div className="h-4 w-1/3 bg-gray-700 rounded mb-2"/>
                    <div className="h-8 w-1/2 bg-gray-700 rounded"/>
                  </div>))) : error ? (<div className="col-span-4 text-center text-red-400 py-8">{error}</div>) : aiMetrics.length > 0 ? (aiMetrics.map((m, i) => (<div key={i} className="h-[110px] min-w-[200px]" style={{ contain: 'strict', contentVisibility: 'auto' }}>
                    <MetricCard title={m.label} value={m.value} delta={m.trend}/>
                  </div>))) : (Array.from({ length: 3 }).map((_, i) => (<div key={i} className="glass-card p-5 rounded-2xl shadow-lg border border-white/6 flex flex-col h-[110px] min-w-[200px] bg-gray-800/20">
                    <div className="text-xs text-gray-400">No data</div>
                    <div className="text-2xl font-semibold text-gray-600 mt-2">--</div>
                  </div>)))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12" style={{ minHeight: '130px', contain: 'layout' }}>
              {loading ? (Array.from({ length: 4 }).map((_, i) => (<div key={i} className="glass-card p-5 rounded-2xl shadow-lg border border-white/6 flex flex-col h-[110px] min-w-[200px] animate-pulse bg-gray-800/40">
                    <div className="h-4 w-1/3 bg-gray-700 rounded mb-2"/>
                    <div className="h-8 w-1/2 bg-gray-700 rounded"/>
                  </div>))) : error ? (<div className="col-span-4 text-center text-red-400 py-8">{error}</div>) : otherMetrics.length > 0 ? (otherMetrics.map((m, i) => (<div key={i} className="h-[110px] min-w-[200px]" style={{ contain: 'strict', contentVisibility: 'auto' }}>
                    <MetricCard title={m.label} value={m.value} delta={m.trend}/>
                  </div>))) : (Array.from({ length: 4 }).map((_, i) => (<div key={i} className="glass-card p-5 rounded-2xl shadow-lg border border-white/6 flex flex-col h-[110px] min-w-[200px] bg-gray-800/20">
                    <div className="text-xs text-gray-400">No data</div>
                    <div className="text-2xl font-semibold text-gray-600 mt-2">--</div>
