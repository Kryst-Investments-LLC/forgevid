...
import { Scissors, Crop, RotateCw, Repeat, Eraser, Type, Music, ImageIcon, Video, Mic, Layers, Palette, Zap, Filter, } from "lucide-react";
...
export function ToolPanel() {
    return (<div className="p-4 space-y-6">
      <div>
        ...
        {toolCategories.map((category, categoryIndex) => (<div key={category.title} className="mb-6">
            <h3 className="text-sm font-medium text-muted-foreground mb-3">{category.title}</h3>
            <div className="grid grid-cols-2 gap-2">
              {category.tools.map((tool) => (<Button key={tool.name} variant="outline" className="h-auto p-3 flex flex-col items-center gap-2 hover:bg-accent bg-transparent">
