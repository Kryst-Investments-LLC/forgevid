...
const StoryboardingPanel = () => {
    ...
    return (<div className="p-6 bg-white rounded shadow" style={{ minHeight: '400px', contain: 'layout' }}>
      <h2 className="text-xl font-bold mb-4">AI-Driven Storyboarding</h2>
      <textarea className="w-full border rounded p-2 mb-4" rows={6} placeholder="Enter your video script..." value={script} onChange={e => setScript(e.target.value)}/>
      <button className="bg-blue-700 text-white px-4 py-2 rounded" onClick={handleGenerate} disabled={loading || !script}>
        {loading ? 'Generating...' : 'Generate Storyboard'}
      </button>
      {error && <div className="text-red-500 mt-2">{error}</div>}
      {storyboard && (<div className="mt-6">
          <h3 className="font-semibold mb-2">Storyboard Scenes</h3>
          <ul className="space-y-2">
            {storyboard.scenes.map(scene => (<li key={scene.id} className="border rounded p-2 bg-gray-50">
