...
import { Upload, Search, Grid3X3, List, FolderPlus, MoreHorizontal, Download, Heart, Play, ImageIcon, Video, Music, FileText, Folder, Star, Eye, } from "lucide-react";
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <Input placeholder="Search media..." className="pl-10 w-64" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
              </div>
          ...
          <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button variant={viewMode === "grid" ? "default" : "outline"} size="sm" onClick={() => setViewMode("grid")} className={viewMode === "grid" ? "" : "bg-transparent"}>
                  <Grid3X3 className="h-4 w-4"/>
                </Button>
                <Button variant={viewMode === "list" ? "default" : "outline"} size="sm" onClick={() => setViewMode("list")} className={viewMode === "list" ? "" : "bg-transparent"}>
              ...
              <div className="w-64 space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    <Button variant={selectedFolder === "all" ? "default" : "ghost"} className="w-full justify-start" onClick={() => setSelectedFolder("all")}>
                      <Folder className="h-4 w-4 mr-2"/>
                      All Media
                    </Button>
                    {folders.map((folder) => (<Button key={folder.name} variant={selectedFolder === folder.name ? "default" : "ghost"} className="w-full justify-between" onClick={() => setSelectedFolder(folder.name)}>
              ...
              <div className="flex-1">
                <TabsContent value={selectedTab} className="mt-0">
                  {viewMode === "grid" ? (<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                      {filteredMedia.map((item) => (<Card key={item.id} className="group hover:shadow-lg transition-shadow">
                          <div className="relative">
                            <img src={item.thumbnail || "/placeholder.svg"} alt={item.name} width={160} height={64} className="w-full h-32 object-cover rounded-t-lg aspect-[5/2]"/>
                            ...
                            {item.isFavorite && (<Heart className="absolute top-2 right-2 h-4 w-4 fill-red-500 text-red-500"/>)}
                      ...
                      <CardContent className="p-0">
                        <div className="space-y-1">
                          {filteredMedia.map((item) => (<div key={item.id} className="flex items-center gap-4 p-4 hover:bg-muted/50 transition-colors">
                              <img src={item.thumbnail || "/placeholder.svg"} alt={item.name} width={48} height={48} className="w-12 h-12 object-cover rounded aspect-square"/>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  {getMediaIcon(item.type)}
                                  <h4 className="font-medium truncate">{item.name}</h4>
                                  {item.isFavorite && <Heart className="h-4 w-4 fill-red-500 text-red-500"/>}
                                  {item.isStock && (<Badge variant="secondary" className="text-xs">
                                      Stock
                                    </Badge>)}
                                </div>
