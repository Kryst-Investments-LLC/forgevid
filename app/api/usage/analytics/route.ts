...
async function generateAnalytics(userId: string, days: number) {
  ...
  // Run all queries in parallel with minimal field selection
  const [subscription, usageRecords, videos, collaboratorCount] = await Promise.all([
    ...
    // Use aggregate count instead of fetching all rooms + members
  ...
  const totalCollaborators = collaboratorCount
