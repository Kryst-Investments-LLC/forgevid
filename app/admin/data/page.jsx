...
import { Database, Download, Upload, Archive, Trash2, Shield, HardDrive, RefreshCw, AlertTriangle, CheckCircle, FileText, Settings, } from "lucide-react";
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {backups.map((backup) => (<TableRow key={backup.id}>
                      <TableCell className="font-medium">{backup.name}</TableCell>
                      <TableCell>
                        <Badge variant={backup.type === "full" ? "default" : "secondary"}>{backup.type}</Badge>
                      </TableCell>
                      <TableCell>
                        {backup.status === "completed" && (<Badge variant="default">
                            <CheckCircle className="h-3 w-3 mr-1"/>
                            Completed
                          </Badge>)}
                        {backup.status === "in_progress" && (<div className="flex items-center space-x-2">
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {dataRetentionPolicies.map((policy) => (<TableRow key={policy.id}>
                      ...
                      <TableCell>
                        {policy.autoDelete ? (<Badge variant="default">Enabled</Badge>) : (<Badge variant="secondary">Manual</Badge>)}
                      </TableCell>
                      <TableCell>
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {exportRequests.map((request) => (<TableRow key={request.id}>
                      <TableCell className="font-medium">{request.requestedBy}</TableCell>
                      <TableCell>
                        <Badge variant="outline">{request.dataType}</Badge>
                      </TableCell>
                      <TableCell>
                        {request.status === "completed" && (<Badge variant="default">
                            <CheckCircle className="h-3 w-3 mr-1"/>
                            Completed
                          </Badge>)}
                        {request.status === "processing" && (<div className="flex items-center space-x-2">
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          {request.status === "completed" && (<Button variant="outline" size="sm">
                              <Download className="h-4 w-4"/>
                            </Button>)}
                          <Button variant="outline" size="sm">
